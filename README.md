...
| Solver | Strategy | Avg Length | Avg Steps |
| :----: | :------: | :--------: | :-------: |
...
| [DQN (TF)][doc-dqn] | Deep Q-Network (TensorFlow) | 24.44 / 64 | 131.69 |
| DQN (PyTorch) | Double DQN, requires training | -- | -- |

Results for the first three solvers averaged over 1000 episodes. The PyTorch DQN solver is untested until retrained -- see [Training](#training).
...
Two DQN implementations are included.

**TensorFlow version** (`-s dqn`): Uses a convolutional network with four conv layers (32, 64, 128, 256 filters) and two fully connected layers, trained with three simultaneous optimizations: Double DQN for stable target computation, Prioritized Experience Replay to focus learning on surprising transitions, and Dueling Network Architecture to separate state value from action advantage.

**PyTorch version** (`-s dqn_torch`): Two conv layers (32, 64 filters) followed by two fully connected layers (256, 128 units), trained with Double DQN and soft target network updates. Epsilon decays linearly from 1.0 to 0.01 over 50k episodes.

Both versions use relative direction mode: actions are left, straight, or right relative to the snake's current heading rather than absolute map directions. This leverages rotational symmetry -- the network sees equivalent game states as equivalent inputs regardless of which way the snake is facing.

The state vector has two components:
- **Grid channels (8x8x4):** binary encodings of food position, body segments (normalized by snake length), head position, and danger cells (body segments that are not the tail)
- **Extra features (9):** one-hot current direction (4), one-hot food direction (4), normalized Manhattan distance to food (1)

The DQN solvers plateau below the classical solvers. The reward for each food piece is separated from the eventual collision by hundreds of steps, making credit assignment difficult.
...
**Why two DQN implementations?** The TensorFlow solver uses an older but more feature-complete training setup (Prioritized Experience Replay, Dueling Networks). The PyTorch solver uses a cleaner, more modern codebase with Double DQN and soft target updates, and is easier to extend and retrain.
